...
public class CustomerManagementSteps {
	...
	static String name;  //this is null as of now so we can put any name down there in the code without give us 
	                     //any problem
	...
	// @newCustomerBTNTest scenario end -
		...
		public void i_should_see_the_customer_list_table() throws InterruptedException {
			...
			if (customerPage.customer_page_NoCustomersYet_text.isDisplayed()) {//if you see "no customer yet"
				customerPage.customer_page_NewCustomerBTN.click();//click on "new customer" button
				utils.waitForElementToBeVisible(customerPage.customer_page_BasicInfo_DisplayName_Field);//wait
				customerPage.customer_page_BasicInfo_DisplayName_Field.sendKeys("Student");//put name "student"
				customerPage.customer_page_NewCustomerSubmit_BTN.click();//click on "save customer" button
				Thread.sleep(1000);
				dashPage.customers_tab.click();//click on "customers"
                //Thread.sleep(1000);
				utils.waitForElementToBeVisible(customerPage.customer_page_Customers_TextHeader);//wait for new page 
				Assert.assertTrue(customerPage.customer_page_CustomerTable.isDisplayed());//verify we are there
			} else {
